...
export default function Home() {
  ...
  async function submitQuery() {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`Backend a raspuns cu status ${res.status}`);
      ...
      const message =
        err instanceof TypeError
          ? "Nu s-a putut contacta backend-ul. Verifica daca API-ul ruleaza."
          : err instanceof Error
            ? err.message
            : "Eroare necunoscuta la interogarea backend-ului.";
      setError(message);
    } finally {
      setLoading(false);
    }
    ...
    <div className="mx-auto flex min-h-screen max-w-2xl flex-col gap-4 px-4 py-8 font-sans">
      ...
      >
        <input
          ...
          placeholder="Intreaba despre 10-K-urile Apple, Microsoft sau Google..."
        ...
        <div className="flex flex-col gap-3 rounded border border-zinc-200 p-4 dark:border-zinc-800">
          ...
          <p className="whitespace-pre-wrap text-sm leading-6">
            {result.answer
              ...
              : "Nu s-a putut genera un raspuns."}
          </p>

          <div className="flex flex-wrap items-center gap-4 text-xs text-zinc-500 dark:text-zinc-400">
            {result.latency_ms != null && <span>Latenta: {result.latency_ms} ms</span>}
            ...
            {result.langsmith_trace_id && (
              <button onClick={copyTraceId} className="underline hover:no-underline">
                {copied ? "Copiat!" : `Trace ID: ${result.langsmith_trace_id}`}
              </button>
            )}
          </div>
          ...
          {result.retrieved_chunks.length > 0 && (
            <div>
              <button onClick={() => setChunksOpen((o) => !o)} className="text-xs font-medium underline">
                {chunksOpen ? "Ascunde" : "Arata"} fragmentele recuperate ({result.retrieved_chunks.length})
              </button>
              {chunksOpen && (
